        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            ...
            <div class="flex items-center gap-3">
                <div class="flex items-center gap-2 text-xs text-slate-500">
                    ...
                    <input type="number" placeholder="30" min="5" max="300" class="w-14 text-xs border border-gray-200 rounded-lg px-2 py-1 outline-none focus:ring-2 focus:ring-[#2979FF] text-center" />
                    <span>sec</span>
                </div>
                <div class="flex items-center gap-2 text-xs text-slate-500">
                    ...
                    <input type="number" placeholder="10" min="0" class="w-14 text-xs border border-gray-200 rounded-lg px-2 py-1 outline-none focus:ring-2 focus:ring-[#2979FF] text-center" />
                    <span>pts</span>
                </div>
                <button onclick="deleteQuestion(this)" class="text-slate-300 hover:text-red-400 transition">
        ...
        <div class="px-6 py-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 choices-container">
                ...
                <div class="choice-item flex items-center gap-3 p-3 border-2 border-gray-100 rounded-xl hover:border-[#2979FF]/30 transition">
                    <button type="button" onclick="setCorrect(this)" class="correct-btn w-6 h-6 rounded-full border-2 border-gray-300 flex-shrink-0 flex items-center justify-center transition hover:border-green-400"></button>
            ...
            <p class="text-xs text-slate-400 mt-3"><span class="text-green-500 font-semibold">●</span> Click the circle next to the correct answer</p>
        </div>`;
...
function renumberQuestions() {
    document.querySelectorAll('.question-card').forEach((card, i) => {
        card.querySelector('span').textContent = `Question ${i + 1}`;
    });
    questionCount = document.querySelectorAll('.question-card').length;
}

function setCorrect(btn) {
    const card = btn.closest('.choices-container');
    // Clear all in this card
    card.querySelectorAll('.correct-btn').forEach(b => {
        b.classList.remove('bg-green-500', 'border-green-500');
        b.innerHTML = '';
        b.closest('.choice-item').classList.remove('border-green-400', 'bg-green-50');
        b.closest('.choice-item').classList.add('border-gray-100');
    });
    // Mark selected
    btn.classList.add('bg-green-500', 'border-green-500');
    btn.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3 fill-white" viewBox="0 0 24 24"><path d="M9 16.17 4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>`;
    btn.closest('.choice-item').classList.add('border-green-400', 'bg-green-50');
    btn.closest('.choice-item').classList.remove('border-gray-100');
}
